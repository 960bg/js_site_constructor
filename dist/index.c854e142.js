...
<hr>
`}n=new URL("homer.432f1a35.jpg",import.meta.url).toString();class f{constructor(t,e){this.value=t,this.options=e}toHtml(){throw Error("Метод toHtml должен быть реализован")}}const m={ImageBlock:class extends f{constructor(t,e){super(t,e)}toHtml(){return l(a(this))}},ColumnsBlock:/*  Блок работает с массивом данных, 
если в value= строка то разделитель для массива = "," */class extends f{constructor(t,e){super(t,e)}toHtml(){let{tag:t="h1",styles:e}=this.options,r="string"==typeof this.value?this.value.split(","):this.value,o=r=>c(// если пришло изображение тогда 
    ...
    let r=function(t,e,r="img",o="Картинка"){return"string"!=typeof t&&console.log("stringToImg(): В параметре должна быть передана строка"),{value:t,options:{tag:r,alt:o,styles:e}}}(t,e);return a(r)}(r),e,t),s=r.map(t=>i(o(t))).join("");return l(s)}},TextBlock:class extends f{constructor(t,e){super(t,e)}toHtml(){let{tag:t="p",styles:e}=this.options;return l(i(c(this.value,e,t)))}},TitleBlock:class extends f{constructor(t,e){super(t,e)}toHtml(){let{tag:t="h1",styles:e}=this.options;return l(i(c(this.value,e,t)))}}},p=[new m.TitleBlock("Конструктор сайтов на JS",{tag:"h2",styles:{background:"linear-gradient(to right, #ff0099, #493240)",color:"#fff",padding:"0.7em"}}),new m.TextBlock("HTML страницу формирует JS, исходные данные для блоков записаны в файле model.js.",{tag:"p",styles:{background:"linear-gradient(to left, #f2994a, #f2c94c)",color:"#fff",padding:"0.5em","font-weight":"bold"}}),new m.ColumnsBlock(["файл templates.js - функции обертки в HTML объектов из файла model.js","файл utils.js - функции вызываемые из templates.js","файл blocks.js - классы для объектов блоков из model.js"],{tag:"p",styles:{background:"linear-gradient(to bottom, #8e2de2, #4a00e0)",color:"#fff",padding:"0.5em","font-weight":"bold"}}),new m.ImageBlock((t=n)&&t.__esModule?t.default:t,{tag:"img",alt:"Картинка Гомер Симпсон",styles:{width:"150px",height:"auto",border:"solid 5px",background:"linear-gradient(to top, #00e5ff, #4761bf)",display:"flex","justify-content":"center",color:"#fff",padding:"0.5em"}}),new m.ColumnsBlock(["Стили: для сетки используется bootstrap.","Стили: остальные стили прописываются в файле model.js для каждого блока в секции styles","HTML страницу формирует JS"],{tag:"p",styles:{background:"linear-gradient(to right, #00e5ff, #4761bf)",color:"#fff",padding:"0.5em"}})];// export const model = [
// {
//   type: 'f_title',
